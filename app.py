...
# Render/Railway에 배포 시 환경 변수 설정이 필수입니다.
try:
    ...
    # API 키가 없거나 라이브러리 초기화 실패 시 더미 모드 활성화
...
NEGATIVE_PROMPT = "blurry, distorted, ugly, bad anatomy, realistic, 3D, complex background, text, extra limbs, multiple characters"
...
def generate_image_from_gemini(prompt, negative_prompt):
    """
    Gemini API를 호출하여 이미지를 생성하고 PIL Image 객체를 반환합니다.
    """
    if client is None:
        # API 키가 설정되지 않은 경우 더미 이미지 반환
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            output_prompt = gr.Textbox(label="AI 전달 프롬프트 (개발자 확인용)", interactive=False, visible=True) # 디버깅을 위해 보이게 설정
...
if __name__ == "__main__":
    # Render/Railway 배포를 위해 listen 주소를 0.0.0.0으로 설정
